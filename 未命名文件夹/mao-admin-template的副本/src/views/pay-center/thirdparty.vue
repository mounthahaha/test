<template>
  <el-card v-loading="loading" class="thirdparty">
    ...
    <el-table :data="listData" style="width: 100%" border stripe>
      ...
      <el-table-column prop="tradeNo" label="第三方返回订单号" width="150" />
...
import { getShowPayOrderList, ShowPayOrderFail, ShowPayOrderSuccess, getTypeList } from '@/api/pay-center';
...
const changeSwitchStatus = (val: number, id: number, index: number) => {
  if (val === 1) {
    ShowPayOrderSuccess(id).then(() => {
      ElMessage.success('修改成功')
       listData.value[index].status = 1
    })
  } else {
    ShowPayOrderFail(id).then(() => {
      ElMessage.success('修改成功')
